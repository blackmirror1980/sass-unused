...
function findUnused(srcFiles, resolver) {
  ...
  srcFiles.forEach(path => {
    ...
    // Variables
    declaredVars = declaredVars.concat(gatherDeclaredVars(parseTree).map(function(declaredVar) {
      return {
        type: 'variable',
        path: path,
        value: declaredVar
    ...
    // Mixins
    declaredMixins = declaredMixins.concat(gatherDeclaredMixins(parseTree).map(function(declaredMixin) {
      return {
        type: 'mixin',
        path: path,
        value: declaredMixin
    ...
    // Functions
    declaredFunctions = declaredFunctions.concat(gatherDeclaredFunctions(parseTree).map(function(declaredFunction) {
      return {
        type: 'function',
        path: path,
  ...
  const unusedVars = declaredVars.filter(ident => !usedVars.has(ident.value));
  const unusedMixins = declaredMixins.filter(ident => !usedMixins.has(ident.value));
  const unusedFunctions = declaredFunctions.filter(ident => !usedFunctions.has(ident.value));
