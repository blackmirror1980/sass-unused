...
function findUnused(srcFiles, resolver) {
  ...
  srcFiles.forEach(path => {
    ...
    // Variables
    declaredVars = declaredVars.concat(gatherDeclaredVars(parseTree, src));
    gatherUsedVars(parseTree).forEach(ident => usedVars.add({
      type: 'variable',
      path: src,
      value: ident
    }));
    ...
    // Mixins
    declaredMixins = declaredMixins.concat(gatherDeclaredMixins(parseTree, src));
    gatherUsedMixins(parseTree).forEach(ident => usedMixins.add({
      type: 'mixin',
      path: src,
      value: ident
    }));
    ...
    // Functions
    declaredFunctions = declaredFunctions.concat(gatherDeclaredFunctions(parseTree, src));
    gatherUsedFunctions(parseTree).forEach(ident => usedFunctions.add({
      type: 'function',
      path: src,
      value: ident
    }));
